...
BOT_BUILD = "factions-table-external-v4"
...
def build_faction_table(stats):
    """
    Klassische Tabellenansicht im Codeblock.
    Gegenüber der ursprünglichen Version ist nur der Abstand zwischen
    Volk und Spiele etwas reduziert, damit die Tabelle auf Mobilgeräten
    seltener umbricht.
    """
    header = f"{'Volk':<14}{'Spiele':>6} {'Winrate':>8}  Top-Spieler"
    divider = "-" * len(header)
    ...
    for row in stats:
        ...
        if len(top_players) > 22:
            top_players = top_players[:19] + "..."

        # Top-Spieler nur anzeigen, wenn mindestens 2 Spiele mit diesem Volk vorhanden sind.
        # Externe Spieler werden bereits in get_faction_stats aus dieser Top-Spieler-Zählung entfernt.
        ...
        lines.append(
            f"{faction:<14}{games:>6} {winrate:>8}  {top_text}"
        )
...
async def factions(interaction: discord.Interaction):
    ...
    embed.set_footer(
        text=f"Sortiert nach Spielen. Winrate = Siege / Spiele. Externe zählen nicht als Top-Spieler. Build: {BOT_BUILD}"
    )
